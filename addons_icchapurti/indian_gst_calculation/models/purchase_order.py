...
class CustomPurchaseOrder(models.Model):
    """
        Adds three GST amount columns to purchase order model. 
    """
                ...
                for tax in line.taxes_id:
                    if tax.amount_type == "group":
                        ...
                        if order.order_id.CGST_SGST == True:
                            ...
                            line.purchase_amount_CGST = round(line.price_subtotal * line.purchase_CGST / 100, 2)
                            line.purchase_amount_SGST = round(line.price_subtotal * line.purchase_SGST / 100, 2)
                        elif order.order_id.IGST == True:
                            I_GST_percent = total_gst
                            line.purchase_IGST = I_GST_percent
                            line.purchase_amount_IGST = round(line.price_subtotal * line.purchase_IGST / 100, 2)
                        elif order.order_id.UTGST == True:
                            UT_GST_percent = total_gst
                            line.purchase_UTGST = UT_GST_percent
                            line.purchase_amount_UTGST = round(line.price_subtotal * line.purchase_UTGST / 100, 2)
                        else:
